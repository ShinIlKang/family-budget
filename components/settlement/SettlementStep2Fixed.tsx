...
import type { FixedItem, FixedItemGroup, MonthYear } from '@/types'
...
const FIXED_GROUPS = FIXED_ITEM_GROUPS.filter(g => g !== '저축/투자')

interface Props {
  settlementMonth: MonthYear
  salary: number
  onNext: (fixedTotal: number) => void
  onBack: () => void
}

export default function SettlementStep2Fixed({ settlementMonth, salary, onNext, onBack }: Props) {
  const { data: session } = useSession()
  const [items, setItems] = useState<FixedItem[]>([])
  const [showForm, setShowForm] = useState(false)
  const [name, setName] = useState('')
  const [amount, setAmount] = useState('')
  const [group, setGroup] = useState<FixedItemGroup>('구독/서비스')
  ...
  async function handleAdd() {
    ...
    try {
      await createFixedItem(
        { name: name.trim(), amount: amt, group_name: group, billing_day: null, payment_method: null, memo: null, is_active: true, year: settlementMonth.year, month: settlementMonth.month },
        session?.user.id ?? ''
      )
      setName('')
      setAmount('')
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="sticky top-0 z-10 bg-gray-50 px-4 pt-4 pb-3">
        <div className="mb-4">
          <SettlementHeader settlementMonth={settlementMonth} step={2} />
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 px-4 py-3 text-sm mb-3">
          ...
          <div className="flex justify-between font-medium text-red-500 mt-1">
            <span>고정비용</span><span>- {formatAmount(total)}원</span>
          </div>
        </div>

        <div className="flex items-center justify-between">
          <p className="text-sm font-medium text-gray-700">고정비용 항목 ({items.length}개)</p>
      ...
      {showForm && (
        <div className="bg-gray-50 mx-4 rounded-xl p-4 flex flex-col gap-3 mb-2">
          ...
          <select
            value={group}
            onChange={e => setGroup(e.target.value as FixedItemGroup)}
            className="border border-gray-300 rounded-lg px-3 py-2 text-sm"
          >
      ...
      <div className="px-4">
        {items.length === 0 ? (
          <p className="text-sm text-gray-400 text-center py-6">고정비용 항목이 없습니다</p>
        ) : (
